<template>
  <header
    :class="[
      scrolled && !mobileMenuOpen ? 'bg-white shadow-xl' : 'bg-transparent shadow-none',
      fixedNav ? 'fixed top-0 left-0 w-full' : 'absolute top-0 left-0 w-full'
    ]"
    class="z-10"
  >
    <nav class="mx-auto flex max-w-screen-xl items-center justify-between p-6 px-4 md:px-20">
      ...
      <PopoverGroup class="hidden lg:flex lg:gap-x-12">
        <Popover class="relative">
          <PopoverButton
            class="flex items-center gap-x-1 text-base font-bold leading-6 text-gray-900 hover:text-teal-500"
            :class="scrolled ? 'text-gray-900' : 'text-white'"
          >
            대회 일정
...
const handleScroll = () => {
  ...
  fixedNav.value = window.scrollY > 1
}
